...
exports.createEnsureTypeItemsNode =
  exports.createNodeId =
  exports.matchArrayItemsNode =
  exports.hasNodeByComponentPath =
  exports.findNodeByComponentPath =
  exports.queryNodesByComponentPath =
  exports.includesComponent =
  exports.matchChildComponent =
...
var includesComponent = function (node, names, target) {
  return names.some(function (name) {
    return exports.matchComponent(node, name, target)
  })
}
exports.includesComponent = includesComponent
var queryNodesByComponentPath = function (node, path) {
  if ((path === null || path === void 0 ? void 0 : path.length) === 0) return []
  ...
  return exports.matchComponent(node, path[0])
    ? node.children.reduce(function (buf, child) {
        return buf.concat(
          exports.queryNodesByComponentPath(child, path.slice(1))
        )
      }, [])
    : []
}
...
var findNodeByComponentPath = function (node, path) {
  ...
  if (exports.matchComponent(node, path[0])) {
    for (var i = 0; i < node.children.length; i++) {
      var next = exports.findNodeByComponentPath(
        node.children[i],
        path.slice(1)
      )
      if (next) {
...
var hasNodeByComponentPath = function (node, path) {
  return !!exports.findNodeByComponentPath(node, path)
}
...
var matchArrayItemsNode = function (node) {
  ...
  return (
    ((_b =
      ...
      : _b.type) === 'array' &&
    ((_d =
      (_c = node === null || node === void 0 ? void 0 : node.parent) === null ||
      _c === void 0
        ? void 0
...
var createEnsureTypeItemsNode = function (type) {

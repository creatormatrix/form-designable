...
var useDragDropEffect = function (engine) {
  engine.subscribeTo(events_1.DragStartEvent, function (event) {
    if (engine.cursor.type !== models_1.CursorType.Normal) return
        ...
        if (node_1) {
          ...
          if (
            ...
            moveHelper.dragStart({
              dragNodes: models_1.TreeNode.sort(validSelected),
            })
          } else {
  ...
  engine.subscribeTo(events_1.DragMoveEvent, function (event) {
    if (engine.cursor.type !== models_1.CursorType.Normal) return
    if (engine.cursor.dragType !== models_1.CursorDragType.Move) return
    var target = event.data.target
    var el =
      target === null || target === void 0
        ? void 0
  ...
  engine.subscribeTo(events_1.DragStopEvent, function () {
    ...
    engine.workbench.eachWorkspace(function (currentWorkspace) {
      ...
      if (dragNodes.length && closestNode && closestDirection) {
        if (
          closestDirection === models_1.ClosestPosition.After ||
            ...
            selection.batchSafeSelect(
              closestNode.insertAfter.apply(
                ...
                __spreadArray(
                  [],
                  __read(
                    models_1.TreeNode.filterDroppable(
                      dragNodes,
                      closestNode.parent
                    )
                  ),
          ...
          closestDirection === models_1.ClosestPosition.Before ||
          ...
          closestDirection === models_1.ClosestPosition.Inner ||
            ...
            selection.batchSafeSelect(
              closestNode.append.apply(
                ...
                __spreadArray(
                  [],
                  __read(
                    models_1.TreeNode.filterDroppable(dragNodes, closestNode)
                  ),
                  false
                )
              )

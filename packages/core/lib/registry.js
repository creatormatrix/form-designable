...
var reSortBehaviors = function (target, sources) {
  ...
  ;(0, shared_1.each)(sources, function (item) {
    if (!item) return
    if (!(0, externals_1.isBehaviorHost)(item)) return
...
var DESIGNER_LANGUAGE_STORE = reactive_1.observable.ref(
  (0, internals_1.getBrowserLanguage)()
)
  ...
  getDesignerMessage: function (token, locales) {
    ...
    if (!locale) {
      for (var key in DESIGNER_LOCALES_STORE.value) {
        var message = path_1.Path.getIn(
          DESIGNER_LOCALES_STORE.value[key],
          (0, internals_1.lowerSnake)(token)
        )

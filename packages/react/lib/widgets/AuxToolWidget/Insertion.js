...
exports.Insertion = (0, reactive_react_1.observer)(function () {
  ...
  var createInsertionStyle = function () {
    ...
    if (
      closestDirection === core_1.ClosestPosition.Before ||
      ...
      baseStyle.transform = 'perspective(1px) translate3d('
        .concat(closestRect.x, 'px,')
        .concat(closestRect.y, 'px,0)')
    } else if (
      closestDirection === core_1.ClosestPosition.After ||
      closestDirection === core_1.ClosestPosition.ForbidAfter
    ) {
      baseStyle.width = 2
      baseStyle.height = closestRect.height
      baseStyle.transform = 'perspective(1px) translate3d('
        .concat(closestRect.x + closestRect.width - 2, 'px,')
        .concat(closestRect.y, 'px,0)')
    } else if (
      ...
      if (isInlineLayout) {
        ...
        baseStyle.transform = 'perspective(1px) translate3d('
          .concat(closestRect.x, 'px,')
          .concat(closestRect.y + closestRect.height - 2, 'px,0)')
      }
    } else if (
